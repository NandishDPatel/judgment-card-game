...
function notifyPlayer(room, playerId, message) {
  wss.clients.forEach((ws) => {
    const meta = clientMeta.get(ws);
    if (!meta || meta.roomCode !== room.code || meta.playerId !== playerId) return;
    ws.send(JSON.stringify({ type: 'game:notification', payload: { message } }));
  });
}
  ...
  ws.on('message', async (raw) => {
    try {
      ...
      if (type === 'room:join') {
        ...
        const playerId = addPlayer(room, payload.name);
        clientMeta.set(ws, { roomCode: room.code, playerId });
        await store.saveRoom(room);
        broadcastRoom(room);
        return;
      ...
      if (type === 'room:start') {
        if (meta.playerId !== room.hostId) throw new Error('Only host can start.');
        if (room.players.length !== room.maxPlayers) throw new Error('Waiting for all players.');
        startGame(room);
        setTurnTimer(room);
        await store.saveRoom(room);
        broadcastRoom(room);
      ...
      if (type === 'room:stop') {
        if (meta.playerId !== room.hostId) throw new Error('Only host can stop.');
        room.phase = 'finished';
        setTurnTimer(room);
        await store.saveRoom(room);
        broadcastRoom(room);
      ...
      if (type === 'room:restart') {
        if (meta.playerId !== room.hostId) throw new Error('Only host can restart.');
        room.phase = 'lobby';
        room.round = null;
        room.rounds = [];
        room.roundIndex = 0;
        ...
        room.currentTurnPlayerName = room.players.find((p) => p.id === room.hostId)?.name ?? '';
      ...
      ws.send(JSON.stringify({ type: 'room:error', payload: { message: error.message } }));
    }
  });
...
setInterval(async () => {
  for (const room of store.rooms.values()) {
    ...
    const remaining = Math.max(0, Math.ceil((room.turnExpiresAt - Date.now()) / 1000));
    if (remaining === 0 && !room.turnNotified) {
      room.turnNotified = true;
      notifyPlayer(room, room.currentTurnPlayerId, 'Please play, it\'s your turn.');
    }
    broadcastRoom(room);
  }
}, 1000);
