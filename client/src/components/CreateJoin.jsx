...
export default function CreateJoin({
  ...
  const [showCreateCard, setShowCreateCard] = useState(!inviteMode);
  ...
  return (
    <div className="min-h-screen bg-felt-pattern px-6 py-10">
      <div className="mx-auto max-w-4xl">
          ...
          <div className="flex items-center gap-3">
            {inviteMode ? (
              <button
                ...
                className="rounded-full border border-amber-300/50 bg-amber-200/10 px-4 py-2 text-xs font-semibold text-amber-100 transition hover:bg-amber-200/20 active:scale-[0.98]"
              >
                {showCreateCard ? 'Hide Create Game' : 'Create Game'}
              </button>
            ) : null}
            <button
              onClick={onHelp}
        ...
        <div className={`mt-10 grid gap-6 ${showCreateCard ? 'md:grid-cols-2' : ''}`}>
          {showCreateCard ? (
